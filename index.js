...
function checkRowCol(r, c, symbol, size) {
    ...
    for (let i = 0; i < size; i++) {
        const clickedCellR = document.querySelector(`[data-row="${r}"][data-column="${i}"]`);
        const rowVal = clickedCellR.textContent;
        const clickedCellC = document.querySelector(`[data-row="${i}"][data-column="${c}"]`);
...
function checkDiagonals(symbol, size) {
    ...
    for (let i = 0; i < size; i++) {
        const diagonalL = document.querySelector(`[data-row="${i}"][data-column="${i}"]`);
        const valL = diagonalL.textContent;
        if (valL === symbol) {
            mainDiagonalCount++;
        }
        const diagonalR = document.querySelector(`[data-row="${i}"][data-column="${size - i - 1}"]`);
